...
Both ``pronounce.score.acoustic`` and ``pronounce.score.phoneme`` (text-only) return
...
class PronunciationResult:
    ...
    # ipa_words: per whitespace token, word-local phone alignment for the IPA
    # diagnosis panel (phoneme engine). Each entry:
    #   word, expected, heard, ok (parallel lists), start_sec, end_sec
    # (seconds into the prepared user waveform, or None when untimed).
    # Acoustic/none leave this empty.
    ipa_words: list[dict[str, Any]] = field(default_factory=list)

    # --- Acoustic-engine diagnostics (pronounce.score.acoustic); safe to ignore elsewhere. ---
    ...
    # --- Phoneme-engine diagnostics (pronounce.score.phoneme); safe to ignore elsewhere. ---
